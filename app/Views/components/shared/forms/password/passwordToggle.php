...
<div class="password-toggle mb-3" component="password-toggle" password='visibility'>
    <div class="input-group relative shadow-sm border-gray-200 border-2 rounded-md">
        <input class="form-control w-100 h-[3rem] text-sm lg:text-lg px-2 pt-3 rounded-sm outline-accent focus:outline-solid" type="password" data-password-target
            name="<?= $name ?>"
        ...
        <label class="absolute left-1 top-20 pl-1 text-sm lg:text-lg text-black-700" data-label-toggle component="password-toggle:label" for="<?= $id ?? $name ?>">
            <strong>
                <?= ucfirst($label) ?>
        ...
        <div component="password-toggle:visibility" class="input-group-append absolute w-[.5rem] h-[.5rem] top-20 right-2 z-20 cursor-pointer fill-accent" password-visibility='show'>
        </div>
    </div>
