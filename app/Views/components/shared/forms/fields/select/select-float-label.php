    ...
    <div class="relative shadow-sm border-gray-200 border-2 rounded-md">
        <div class="w-100">
            <select
                ...
                id="<?= !empty($id) ? $id : $name ?>"
                data-label="<?= $label ?>"
                class="form-control w-100 h-[3rem] text-sm lg:text-md pl-2 pr-9 pt-4 rounded-sm outline-theme focus:outline-solid <?= $className ?? null ?>"
            ...
            <?php if (!isset($labelNot)) : ?>
                <label class="absolute left-1 top-4 pl-1 text-sm text-black-300" component="select:label" for="<?= !empty($id) ? $id : $name ?>">
        ...
        <?php if (isset($icon)) : ?>
            <div class="absolute right-0 top-0 h-full pt-2 pr-2">
                <div class="input-group-append text-xl w-[1.5rem] h-[1rem] text-theme">
                    <?= $icon ?>
                </div>
            </div>
        <?php endif; ?>
