...
class GetUseCases
{
    ...
    public function execute(array $payload)
    {
        ...
        $foundDeletedUsers = $usersModel->select("id")->onlyDeleted()->findAll();

        if (\count($foundDeletedUsers) == 0)
            return [];

        $data = $userEntity->toArray(true);
        $data['in_ids'] = \array_map(fn(UserEntity $user) => $user->getId(), $foundDeletedUsers);

        $foundUserRole = $usersRolesModel->limit($limit, $startIndexRegister)->getUsersWithRole($data, []);
