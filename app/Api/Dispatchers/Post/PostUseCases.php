...
namespace App\Api\Dispatchers\Post;
...
class PostUseCases
{
    ...
    public function execute(array $payload)
    {
        ...
        if (\count($foundPlatforms) == 0)
            throw new Exceptions("Api.dispatchers.invalid.platforms", BAD_BUSINESS_RULES);

        if (!$messagesDispatcherBusiness->hasContentToSend($payload))
            throw new Exceptions("Api.dispatchers.invalid.not_found_content", BAD_BUSINESS_RULES);

        if (empty($payload['clients']) || !$clientsBusiness->hasClients($payload['clients']))
            throw new Exceptions("Api.dispatchers.invalid.not_found_clients", BAD_BUSINESS_RULES);
        ...
        $startedDate->add(new DateInterval("PT3M"));
        ...
        return (object)[
            "success" => "Api.dispatchers.success.post"
        ];
    }
}
