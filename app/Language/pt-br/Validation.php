<?php

// override core en language system validation or define your own en language validation message
return [
    "max_length" => "{field} só poderá conter até {param} caracteres",
    "min_length" => "{field} precisará conter no mínimo {param} caracteres",
    "invalid_email" => "O e-mail encontra-se com o formato inválido",
    "enum_invalid" => "O valor inserido não é reconhecido ou inválido no sistema.",
    "invalid_json" => "{json} não contém o formato JSON válido",
    "invalid_field" => "O {field} encontra-se inválido ou não foi encontrado",
    "required" => "{field} é obrigatório, mas não foi encontrado.",
    "string" => "O {field} não é uma palavra válida ou encontra-se vazia",
    "valid_url" => "O {field} encontra-se com a url inválida ou incompleta.",
    "in_list" => "Api.invalid.{field}",
    "invalid_index" => "O {field} índice não atende aos critérios mínimos da operação",
    "is_unique" => "{field} está inválido ou registrado.",
    "not_found" => "{field} está com valor inválido.",
    "valid_date" => "{field} não é uma data válida.",
    "already_exists" => "{field} já está utilizado ou não é permitido",
    "boolean" => "{field} não é um booleano válido",
    "login_invalid" => "Verifique se sua senha ou seu e-mail estão corretos."
];
